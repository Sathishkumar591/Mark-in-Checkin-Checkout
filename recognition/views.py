        ...
        for face in faces:
            ...
            if pred != [-1]:
                ...
                if count[pred] == 4 and (time.time()-start[pred]) > 1.2:
                    ...
                    if not present[pred]:  # Check if a picture has already been taken for this person
                        present[pred] = True
                        log_time[pred] = datetime.datetime.now()
                        count[pred] = count.get(pred, 0) + 1
                        print(pred, present[pred], count[pred])
                        # Save face image
                        folder_name = os.path.join('Check-In image', datetime.datetime.now().strftime("%Y-%m-%d"))
                        if not os.path.exists(folder_name):
                            os.makedirs(folder_name)
                        face_img_path = os.path.join(folder_name, f'{person_name}_{datetime.datetime.now().strftime("%H-%M-%S")}.jpg')
                        cv2.imwrite(face_img_path, face_aligned)
            ...
            if pred != [-1]:
                ...
                if count[pred] == 4 and (time.time() - start[pred]) > 1.5:
                    ...
                    if not present[pred]:
                        ...
                        folder_name = os.path.join('Check-Out image', datetime.datetime.now().strftime("%Y-%m-%d"))
                        if not os.path.exists(folder_name):
                            os.makedirs(folder_name)
                        face_img_path = os.path.join(folder_name, f'{person_name}_{datetime.datetime.now().strftime("%H-%M-%S")}.jpg')
                        cv2.imwrite(face_img_path, face_aligned)
...
def view_attendance_date(request):
    ...
    if request.method == 'GET' and 'date' in request.GET:
        ...
        try:
            ...
            messages.error(request, f'Invalid date format: {selected_date_str}. Please select a valid date.')
            return redirect('view-attendance-date')

        time_qs = Time.objects.filter(date=selected_date)
        present_qs = Present.objects.filter(date=selected_date)
        
        if time_qs.exists() or present_qs.exists():
            qs = hours_vs_employee_given_date(present_qs, time_qs)
        else:
            messages.warning(request, f'No records for selected date: {selected_date}.')
            return redirect('view-attendance-date')

    return render(request, 'recognition/view_attendance_date.html', {'selected_date': selected_date, 'qs': qs})
        ...
        if form.is_valid():
            ...
            if username_present(username):
                ...
                if date_to < date_from:
                    ...
                    time_qs = time_qs.filter(date__gte=date_from, date__lte=date_to).order_by('-date')
                    present_qs = present_qs.filter(date__gte=date_from, date__lte=date_to).order_by('-date')
